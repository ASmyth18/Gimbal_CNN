...
Usage examples:
    ...
    # Send via UDP
    python gimbal_tracker.py --telemetry-mode udp --listen-port 22345
"""
...
# -------------------- UDP listener --------------------
def udp_listener(listen_host, listen_port, on_packet, stop_event):
    """
    Listens on UDP (listen_host, listen_port) for JSON telemetry packets.
    Each packet must be UTF-8 JSON with keys: ts, frame_idx, tracks:[{id,cx,cy,w,h,conf,frozen},...]
    Calls on_packet(parsed_dict) for each received packet.
    ...
    try:
        ...
        print(f"[ERROR] UDP listener bind failed {listen_host}:{listen_port}: {e}")
        return
    sock.settimeout(0.5)
    print(f"[INFO] UDP telemetry listener bound to {listen_host}:{listen_port}")
...
def run_tracker(args):
    ...
    latest_tracks = {}  # track_id -> { 'ts': float, 'cx':float, 'cy':float, 'w':int, 'h':int, 'conf':float, 'frozen':int, 'frame_idx':int }
    ...
    # Setup output channel (sending commands)
    ser = None
    sock = None
    udp_addr = None
    if args.mode == "serial":
    ...
    # helper to process incoming UDP telemetry payload
    def on_udp_packet(payload):
        ...
        # each entry expected: id,cx,cy,w,h,conf,frozen
        with lt_lock:
            for t in tracks_list:
                try:
                    ...
                    frozen_flag = int(t.get("frozen", 0))
                except Exception:
                    continue
                latest_tracks[tid] = {
                    'ts': now,
                    'frame_idx': frame_idx,
                    'cx': cx, 'cy': cy, 'w': w, 'h': h,
                    'conf': conf, 'frozen': frozen_flag
                }

    # start UDP listener if requested
    if args.telemetry_mode in ("udp", "both"):
        udp_thread = threading.Thread(target=udp_listener,
                                      args=(args.listen_host, args.listen_port, on_udp_packet, stop_event),
                                      daemon=True)
        udp_thread.start()

    # CSV tail callback (re-uses the same latest_tracks dict)
    ...
    # Start CSV tailer if requested (runs in separate thread so UDP+CSV both work)
    csv_thread = None
    if args.telemetry_mode in ("csv", "both"):
        if not os.path.exists(args.csv):
            print(f"[ERROR] CSV file not found: {args.csv}")
            # If telemetry_mode is both and CSV missing, continue with UDP only.
            if args.telemetry_mode == "csv":
                return
        else:
            csv_thread = threading.Thread(target=tail_csv_realtime, args=(args.csv, handle_rows), daemon=True)
            csv_thread.start()
            print(f"[INFO] CSV tailer started for {args.csv}")

    print(f"[INFO] Gimbal tracker running. telemetry_mode={args.telemetry_mode}, selection={selection}")
    ...
    try:
        while True:
            ...
            now = time.time()
            with lt_lock:
                stale_cutoff = now - STALE_TRACK_SECONDS
                ...
                if current_target_id is None:
                    # no target — optionally send hold / zero motion here
...
# -------------------- CLI --------------------
def parse_args():
    p = argparse.ArgumentParser(description="Real-time gimbal tracker (CSV and/or UDP telemetry)")
    p.add_argument("--csv", default=DEFAULT_CSV, help="Path to tracks_log.csv")
    p.add_argument("--telemetry-mode", choices=["csv", "udp", "both"], default="csv",
                   help="Telemetry source for tracker: csv (default), udp (listen), or both")
    p.add_argument("--listen-host", default="0.0.0.0", help="UDP listen host for telemetry (default 0.0.0.0)")
    p.add_argument("--listen-port", type=int, default=22345, help="UDP listen port for telemetry (default 22345)")
    ...
    p.add_argument("--udp-host", default="127.0.0.1", help="UDP host to send commands to")
    p.add_argument("--udp-port", type=int, default=9000, help="UDP port to send commands to")
